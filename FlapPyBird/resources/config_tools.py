...
def load_images():
    ...
    IMAGES['numbers'] = tuple([pygame.image.load(PATH + '/FlapPyBird/assets/sprites/{}.png'.format(x)).convert_alpha() for x in range(10)])
    ...
    for sprite in sprite_load_list:
        IMAGES[sprite] = pygame.image.load(PATH + '/FlapPyBird/assets/sprites/{}.png'.format(sprite)).convert_alpha()
...
def load_sounds():
    if SOUND_ON:
        ...
        for sound in sound_list:
            SOUNDS[sound] = pygame.mixer.Sound(PATH + '/FlapPyBird/assets/audio/{}'.format(sound) + soundExt)
